...
// File: contracts\libs\IFSwapReferral.sol



pragma solidity 0.6.12;

interface IFSwapReferral {
    /**
     * @dev Record referral.
     */
...
// FSwapToken with Governance.
contract FSwapToken is BEP20 {
    ...
    // Min amount to liquify. (default 500 FSWAPs)
    uint256 public minAmountToLiquify = 500 ether;
    // The swap router, modifiable. Will be changed to FlatSwap's router when our own AMM release
    IUniswapV2Router02 public fswapRouter;
    // The trading pair
    address public fswapPair;
    // In swap and liquify
    ...
    event FSwapRouterUpdated(address indexed operator, address indexed router, address indexed pair);
    ...
    modifier antiWhale(address sender, address recipient, uint256 amount) {
        if (maxTransferAmount() > 0) {
            if (
                ...
                require(amount <= maxTransferAmount(), "FSWAP::antiWhale: Transfer amount exceeds the maxTransferAmount");
            }
        }
        _;
    }
    ...
    /**
     * @notice Constructs the FSwapToken contract.
     */
    constructor() public BEP20("FlatSwap Token", "FSWAP") {
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override antiWhale(sender, recipient, amount) {
        ...
        if (recipient == BURN_ADDRESS || transferTaxRate == 0) {
            ...
            require(taxAmount == burnAmount + liquidityAmount, "FSWAP::transfer: Burn value invalid");

            // default 95% of transfer sent to recipient
            uint256 sendAmount = amount.sub(taxAmount);
            require(amount == sendAmount + taxAmount, "FSWAP::transfer: Tax value invalid");
    ...
    function swapTokensForEth(uint256 tokenAmount) private {
        // generate the flatSwap pair path of token -> weth
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = fswapRouter.WETH();

        _approve(address(this), address(fswapRouter), tokenAmount);

        // make the swap
        fswapRouter.swapExactTokensForETHSupportingFeeOnTransferTokens(
            tokenAmount,
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount) private {
        ...
        // add the liquidity
        fswapRouter.addLiquidityETH{value: ethAmount}(
            address(this),
            tokenAmount,
    ...
    // To receive BNB from fswapRouter when swapping
    ...
    function updateTransferTaxRate(uint16 _transferTaxRate) public onlyOperator {
        require(_transferTaxRate <= MAXIMUM_TRANSFER_TAX_RATE, "FSWAP::updateTransferTaxRate: Transfer tax rate must not exceed the maximum rate.");
    ...
    function updateBurnRate(uint16 _burnRate) public onlyOperator {
        require(_burnRate <= 100, "FSWAP::updateBurnRate: Burn rate must not exceed the maximum rate.");
    ...
    function updateMaxTransferAmountRate(uint16 _maxTransferAmountRate) public onlyOperator {
        require(_maxTransferAmountRate <= 10000, "FSWAP::updateMaxTransferAmountRate: Max transfer amount rate must not exceed the maximum rate.");
     ...
     * Can only be called by the current operator.
     */
    function updateFSwapRouter(address _router) public onlyOperator {
        fswapRouter = IUniswapV2Router02(_router);
        fswapPair = IUniswapV2Factory(fswapRouter.factory()).getPair(address(this), fswapRouter.WETH());
        require(fswapPair != address(0), "FSWAP::updateFSwapRouter: Invalid pair address.");
        emit FSwapRouterUpdated(msg.sender, address(fswapRouter), fswapPair);
    }
    ...
    function transferOperator(address newOperator) public onlyOperator {
        require(newOperator != address(0), "FSWAP::transferOperator: new operator is the zero address");
    ...
    */
        ...
        external
    {
        ...
        require(signatory != address(0), "FSWAP::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "FSWAP::delegateBySig: invalid nonce");
        require(now <= expiry, "FSWAP::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber)
        ...
        returns (uint256)
    {
        require(blockNumber < block.number, "FSWAP::getPriorVotes: not yet determined");
        ...
        internal
    {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying FSWAPs (not scaled);
    ...
    function _writeCheckpoint(
        ...
        internal
    {
        uint32 blockNumber = safe32(block.number, "FSWAP::_writeCheckpoint: block number exceeds 32 bits");
